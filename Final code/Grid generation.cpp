...
int deleted,nn=2,nmax=2,pmax,s,nk,r,convergence=0,N,bmax,cmin,cmax,ffmin,ffmax;
const int cellmax=10000,pnmax=22000,bmin=4;
float ab,sx,sy,pi=3.14159,tolerence=-.1e-4;
...
if(i==505)cycle++;
	...
	if(k==1) {r=2;s=0;}
	if(k==2) {r=0;s=1;}
//if(i==505){printf("jp=%i cell j=%i P[i]=%i k=%i r=%i s=%i\n",jp,j,i,k,r,s);getch();}
//if(i==505){printf("cycle=%i ffmin=%i ffmax=%i poly\n",cycle,ffmin,ffmax);getch();}

//if(i==505){printf("nr=%i ns=%i\n",cell[j].N[r],cell[j].N[s]);getch();}
...
while(convergence==0)
{
	...
	for(int j=1;j<=nmax;j++)//points generator
	{
	 	if(cell[j].a==1)//if triangle is active.
		{
			...
			for(int ii=4;ii<=ffmax;ii++)
			{
				if(d(P[i],P[ii])<.7*P[i].f&&P[i].f>P[ii].f) s=0;
			}

/*bug here	for(int ii=ffmax+1;ii<=i-1;ii++)
			{
			 	if(d(P[i],P[ii])<.7*P[i].f&&P[i].f>P[ii].f) s=0;
	...
	ffmax=i;
	if(ffmax==ffmin-1){printf("No more prospective points left.Solution converged\n");break;} 
	bayeralgo(ffmin,ffmax);//using bayer algorthm for inserting generated points into mesh.
...
{
	ss=0;S=0;
	for(int i=pmax;i<=ffmax;i++)//smoothning
	{
		N=0;sx=0;sy=0;
		for(j=1;j<=nmax;j++) 
		{
		...
		N=0;J=j;
if(k==3||j==nmax+1){printf("error, point %i not found in smothning",i);getch();exit(0);}
		if(k==0) {r=1;s=2;}
		if(k==1) {r=2;s=0;}
		if(k==2) {r=0;s=1;}
		nr=cell[j].N[r];
		...
		S=S+fabs(P[i].x-px);
		S=S+fabs(P[i].y-py);//printf("P[i]=%i\t Sum=%f\n",i,S);
	}
printf("Sum=%e\n",S);
}
...
op = fopen ("temp.txt","w");
for(int j=1;j<=nmax;j++)tri(cell[j]);fclose(op);//output

op = fopen ("CC.txt","w");
fprintf (op,"%i %i\n",cmax-3,ffmax-3);
for(int i=4;i<=ffmax;i++)	fprintf (op,"%f %f\n",P[i].x,P[i].y);
